...
app.get('/api/users',async(req,res)=>{try{const{data,error}=await supabase.from('users').select('username,profile_picture,pfpUrl');if(error)throw error;res.json(data||[])}catch(e){res.status(500).json({error:'Failed to fetch users.'})}});
